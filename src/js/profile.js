$("#update_pass").submit(function (e) {
  e.preventDefault();
  swal("Traitement de la requete en cours...");
  let form = new FormData(this);
  if (form.get("new_1") == form.get("new_2")) {
    $.post(
      "server/?clients-connect",
      { telephone: db.get("user")["telephone"], mot_de_passe: form.get("old") },
      function (data, textStatus, jqXHR) {
        if (data.data.length == 1) {
          $.post(
            "server/?clients-update=" + db.get("user")["id"],
            { mot_de_passe: form.get("new_1") },
            function (data, textStatus, jqXHR) {
              swal(
                "Mot de passe changé avec success! nous allons vous rediriger vers la page de connexion!!"
              );
              deconnect();
            },
            "json"
          ...
          swal("Ancien mot de passe incorect!!");
        }
      },
      "json"
    );
  } else {
    swal("Les nouveaux mots de passe ne sont pas indentique");
  }
});
